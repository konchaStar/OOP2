...
import transport.MotorizedBoat;
...
public class BoatFactory implements Factory{
    ...
    public Optional<Transport> getTransport(HBox box) {
        MotorizedBoat boat = new MotorizedBoat();
        Class<? extends Transport> boatClass = MotorizedBoat.class;
        Map<String, Method> setters = new HashMap<>();
        for(Method setter : Arrays.stream(boatClass.getMethods()).filter(x -> x.getName().indexOf("set") == 0).toList()){
        ...
        for(int i = 1; i < fieldsName.getChildren().size(); i++){
            ...
            if(label.getText().equals("Двигатель")){
                ...
                try {
                    setters.get(label.getText()).invoke(boat, engine);
                } catch (IllegalAccessException e) {
                    e.printStackTrace();
                } catch (InvocationTargetException e) {
                    e.printStackTrace();
                ...
                try {
                    setters.get(label.getText()).invoke(boat, Integer.parseInt(((TextField)fields.getChildren().get(i)).getText()));
                ...
                try {
                    setters.get(label.getText()).invoke(boat, ((TextField)fields.getChildren().get(i)).getText());
                } catch (IllegalAccessException e) {
                    e.printStackTrace();
                } catch (InvocationTargetException e) {
                    e.printStackTrace();
    ...
    public void getFields(HBox box, Optional<? extends Transport> transport) {
        Class<?> boatClass = MotorizedBoat.class;
        List<Method> methods = Arrays.stream(boatClass.getMethods()).filter(x -> x.getName().indexOf("get") == 0).toList();
        ...
        for(Method getter : methods){
            if(getter.isAnnotationPresent(Name.class)){
                ...
                if(getter.getReturnType().equals(Engine.class)){
                    ...
                    for(Method method : Arrays.stream(Engine.class.getMethods()).filter(x -> x.getName().startsWith("get")).toList()){
                        if(method.isAnnotationPresent(Name.class)){
                            ...
                            if(transport.isPresent()){
                                try {
                                    field.setText(method.invoke(getter.invoke((MotorizedBoat)transport.get())).toString());
                                } catch (IllegalAccessException e) {
                                    e.printStackTrace();
                                } catch (InvocationTargetException e) {
                                    e.printStackTrace();
                    ...
                    if(transport.isPresent()) {
                        try {
                            field.setText(getter.invoke((MotorizedBoat)transport.get()).toString());
                        } catch (IllegalAccessException e) {
                            e.printStackTrace();
                        } catch (InvocationTargetException e) {
                            e.printStackTrace();
